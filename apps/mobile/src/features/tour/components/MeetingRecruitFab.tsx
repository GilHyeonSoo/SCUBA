...
const FAB_LABEL_WIDTH = 58;
const FAB_EXPANDED_WIDTH =
  FAB_PADDING_H * 2 + FAB_ICON_SIZE + FAB_LABEL_GAP + FAB_LABEL_WIDTH;
...
export function MeetingRecruitFab({ onPress }: MeetingRecruitFabProps) {
  ...
  const buttonAnimatedStyle = useAnimatedStyle(() => {
    const width = interpolate(
      ...
      [0, 1],
      [FAB_EXPANDED_WIDTH, FAB_SIZE],
      Extrapolation.CLAMP,
    );
    const horizontalPadding = interpolate(
      collapseProgress.value,
      [0, 1],
      [FAB_PADDING_H, (FAB_SIZE - FAB_ICON_SIZE) / 2],
      Extrapolation.CLAMP,
    );
